...
(function(){
  ...
  async function postProjectGantt(projectId, model, reason="gantt-save"){
    ...
    // De Gantt-mutatie staat al in de centrale CWS-state. Gebruik uitsluitend
    // de centrale savequeue en dus één atomische /api/state-writer.
    if(typeof window.CWS?.persistNow !== "function") throw new Error("Centrale CWS-opslag is niet beschikbaar.");
    await window.CWS.persistNow(`direct-gantt-bridge:${reason}`);
    const version = Number(window.CWS?.getRemoteVersion?.() || 0);
    if(window.CWS?.storageStatus){
      window.CWS.storageStatus.lastDirectProjectGanttSaveAt = new Date().toISOString();
      window.CWS.storageStatus.lastDirectProjectGanttSave = { projectId, version, centralized:true, marker:MARKER };
    }
    return { ok:true, projectId, version, centralized:true, marker:MARKER };
  }
  ...
  function frameApi(doc=getFrameDoc()){
    try { return doc?.defaultView?.CWS_GanttFrameAPI || null; } catch(_) { return null; }
  }

  function installPreviewMaterializeGuard(){
    const doc = getFrameDoc();
    const api = frameApi(doc);
    if(!doc || !api?.materializePreview) return false;
    if(doc.__cwsPreviewMaterializeGuardInstalled) return true;
    doc.__cwsPreviewMaterializeGuardInstalled = true;
    const materialize = event => {
      const target = event.target?.closest?.(".bar, #addTaskBtn, #mobileAddTask, #addPhaseBtn, #generateBtn, [data-context-action], #chartPane, #tableRows tr");
      if(!target) return;
      const projectId = getProjectId(doc);
      if(projectId) api.materializePreview(projectId);
    };
    doc.addEventListener("pointerdown", materialize, true);
    doc.addEventListener("click", materialize, true);
    doc.addEventListener("keydown", event => {
      if((event.key === "Enter" || event.key === " ") && event.target?.closest?.("#addTaskBtn, #mobileAddTask, #addPhaseBtn, [data-context-action]")) materialize(event);
    }, true);
    doc.__cwsPreviewMaterializeMarker = "cws-no-eval-preview-materialize-v1";
    return true;
  }
  ...
  function installRevisionActionBridge(){
    ...
    doc.addEventListener("click", async (event) => {
      ...
      if(action === "view" || action === "print"){
        ...
        const api = frameApi(doc);
        api?.closeModal?.();
        api?.setRevisionId?.(rev.id);
        if(action === "print") setTimeout(() => api?.printA3?.(), 180);
        return;
      }
      if(action === "restore"){
