...
(function(){
  ...
  const API_STATE_RESET = "/api/state-reset";
  ...
  const MAX_SAFE_LOGO_DATA_URL_CHARS = 1_600_000;
  const normalizeSafeLogo = (value) => {
    if(!value || typeof value !== "object" || Array.isArray(value)) return null;
    const dataUrl = String(value.dataUrl || "");
    if(dataUrl.length > MAX_SAFE_LOGO_DATA_URL_CHARS) return null;
    const match = /^data:image\/(png|jpeg);base64,[A-Za-z0-9+/]+={0,2}$/.exec(dataUrl);
    if(!match) return null;
    const type = match[1] === "png" ? "image/png" : "image/jpeg";
    const name = String(value.name || (match[1] === "png" ? "logo.png" : "logo.jpg"))
      .slice(0,120).replace(/[<>"'`\\/]/g,"_");
    return {
      dataUrl,
      name,
      type,
      size:Math.max(0, Math.min(Number(value.size || 0) || 0, MAX_SAFE_LOGO_DATA_URL_CHARS)),
      uploadedAt:/^\d{4}-\d{2}-\d{2}T/.test(String(value.uploadedAt || "")) ? String(value.uploadedAt) : new Date().toISOString()
    };
  };
  const safeLogoDataUrl = (value) => normalizeSafeLogo(value)?.dataUrl || "";
  ...
  const loadChunkedRemoteStateBody = async (manifest, version) => {
    ...
    const fetchOne = async (index) => {
      const url = `${API_STATE}?payload=raw-state&chunkIndex=${encodeURIComponent(String(index))}&version=${encodeURIComponent(String(manifest?.dataVersion || manifest?.version || version || 0))}`;
  ...
  const syncDepartments = (st) => {
    ...
    st.company.logo = normalizeSafeLogo(st.company.logo);
  ...
  // Only used as a conservative recovery source gate. Normal D1 hydration must
  // accept every structurally valid planning document, including a brand-new
  // empty database and intentionally small (1-5 project) datasets.
  ...
  const storageAdapter = {
    ...
    async save(snapshot, options={}){
      ...
      const reason = String(options.reason || "state-save");
      const intentionalDestructive = reason === "reset-demo" || reason.startsWith("import:") || /restore|recovery|repair/i.test(reason);
      const remoteSnapshot = createRemoteSaveSnapshot(snapshot);
      if(!intentionalDestructive) protectAgainstCatastrophicOverwrite(remoteSnapshot, "remote save");
      ...
      if(reason === "reset-demo"){
        headers["X-CWS-Destructive-Intent"] = "reset-demo";
        headers["X-CWS-Destructive-Confirm"] = String(remoteVersion);
      }else if(reason.startsWith("import:")){
        headers["X-CWS-Destructive-Intent"] = "import";
        headers["X-CWS-Destructive-Confirm"] = String(remoteVersion);
      }else if(/restore/i.test(reason)){
        headers["X-CWS-Destructive-Intent"] = "restore";
        headers["X-CWS-Destructive-Confirm"] = String(remoteVersion);
      }else if(/recovery|repair/i.test(reason)){
        headers["X-CWS-Destructive-Intent"] = "recovery";
        headers["X-CWS-Destructive-Confirm"] = String(remoteVersion);
      }
      ...
      storageStatus.remoteVersion = remoteVersion;
      storageStatus.label = data?.v82?.chunked ? "Cloudflare D1 - gedeelde planning" : "Cloudflare D1 - gedeelde planning";
    ...
    async reset(){
      if(storageStatus.mode !== "api") return { ok:true, local:true };
      if(storageStatus.stateSource !== "remote-d1") throw new Error("Reset geblokkeerd: D1 is niet de actieve gegevensbron.");
      const response = await fetchWithTimeout(API_STATE_RESET, {
        method:"POST",
        headers:{
          "Content-Type":"application/json; charset=utf-8",
          "Accept":"application/json",
        ...
        body:JSON.stringify({ confirm:"RESET_PLANNING", baseVersion:remoteVersion })
      }, SAVE_FETCH_TIMEOUT_MS, "D1 state-reset");
      const data = await response.json().catch(()=>({}));
      if(response.status === 409){
        const error = new Error(data.error || "State is gewijzigd door een andere gebruiker.");
        error.status = 409;
        error.currentVersion = data.currentVersion;
        throw error;
      }
      if(!response.ok || !data.ok){
        const error = new Error(data.error || `Planning wissen mislukt (${response.status}).`);
        error.status = response.status;
        throw error;
      }
      remoteVersion = Number(data.version || remoteVersion);
      storageStatus.remoteVersion = remoteVersion;
      storageStatus.lastRemoteChunked = false;
      storageStatus.lastRemoteChunkCount = 0;
      return data;
    },
  ...
  const runRemoteSaveOnce = async (reason="user-mutation") => {
    ...
    const intentionalDestructive = reason === "clear-all" || reason === "reset-demo" || reason.startsWith("import:") || /restore|recovery|repair/i.test(reason);
    const validation = validateState(snapshot, { allowDestructive:intentionalDestructive });
    if(!validation.valid){
      const error = new Error(`Opslaan geblokkeerd: ${validation.errors[0] || "state-validatie mislukt."}`);
      error.cwsGuard = "v72-validation-guard";
      throw error;
    }
    if(!intentionalDestructive) protectAgainstCatastrophicOverwrite(snapshot, "remote save");
    if(reason === "clear-all") await storageAdapter.reset();
    else await storageAdapter.save(snapshot, { reason });
    clearRemoteSaveRetry();
  ...
  const validateState = (candidate=state, options={}) => {
    const errors = [];
    const warnings = [];
    const st = candidate || {};
    ...
    if(!options.allowDestructive && recoveryLock?.locked && m.projectCount < Number(recoveryLock.minProjects || 20)){
  ...
  const stateIsValidRemoteDocument = (candidate) => {
    const metrics = stateMetrics(candidate);
    if(!metrics.hasLegacyObjectSchema) return false;
    const validation = validateState(candidate, { allowDestructive:true });
  ...
  const save = ({ userAction=false, reason="state-save" }={}) => {
    ...
    const intentionalDestructive = reason === "clear-all" || reason === "reset-demo" || reason.startsWith("import:") || /restore|recovery|repair/i.test(reason);
    const result = validateState(state, { allowDestructive:intentionalDestructive });
    if(!result.valid){
  ...
  const MAX_STATE_IMPORT_BYTES = 8 * 1024 * 1024;
  const MAX_STATE_IMPORT_NODES = 250000;
  const importByteLength = (value) => new TextEncoder().encode(String(value ?? "")).byteLength;
  const assertImportComplexity = (value) => {
    ...
    while(stack.length){
      ...
      nodes += 1;
      if(nodes > MAX_STATE_IMPORT_NODES) throw new Error("Import bevat te veel onderdelen.");
      if(item.depth > 40) throw new Error("Import is te diep genest.");
      const current = item.value;
      if(typeof current === "string" && current.length > 1600000) throw new Error("Import bevat een te lange tekstwaarde.");
      if(!current || typeof current !== "object") continue;
      if(seen.has(current)) throw new Error("Import bevat een circulaire structuur.");
      seen.add(current);
      if(Array.isArray(current) && current.length > 100000) throw new Error("Import bevat een te grote lijst.");
      const children = Array.isArray(current) ? current : Object.values(current);
      for(let index=children.length-1; index>=0; index-=1) stack.push({ value:children[index], depth:item.depth+1 });
    }
    return value;
  };
  const validateImportedObject = (value) => {
    assertImportComplexity(value);
    const serialized = JSON.stringify(value);
    if(importByteLength(serialized) > MAX_STATE_IMPORT_BYTES) throw new Error("Import is te groot (maximaal 8 MB).");
    return value;
  };

  const extractStateJsonFromAnyText = (input) => {
    if(input && typeof input === "object") return validateImportedObject(input);
    const source = String(input || "");
    if(importByteLength(source) > MAX_STATE_IMPORT_BYTES) throw new Error("Import is te groot (maximaal 8 MB).");
    const raw = source.trim();
    if(!raw) throw new Error("Geen state JSON aangeleverd.");
    try{ return validateImportedObject(JSON.parse(raw)); }catch(error){ if(error?.message && !/JSON/i.test(String(error.message))) throw error; }
    ...
    if(directStart >= 0){
      ...
      if(directEnd > directStart){
        ...
        try{ return validateImportedObject(JSON.parse(candidate)); }catch(_e){}
      }
    }
    ...
    for(const candidate of insertCandidates){
      ...
      if(a >= 0 && b > a){
        try{ return validateImportedObject(JSON.parse(candidate.slice(a,b+1))); }catch(_e){}
      }
    }
    ...
    if(a >= 0 && b > a){
      try{ return validateImportedObject(JSON.parse(unescapeSql(jsonish.slice(a,b+1)))); }catch(_e){}
    }
  ...
  const preserveLocalCandidateBeforeRemoteHydration = (candidate, remoteMetrics={}) => {
    if(!stateHasBusinessData(candidate)) return false;
    const localMetrics = stateMetrics(candidate);
    const remoteProjectCount = Number(remoteMetrics.projectCount || 0);
    const remoteGanttRows = Number(remoteMetrics.ganttRowCount || 0);
    if(localMetrics.projectCount <= remoteProjectCount && localMetrics.ganttRowCount <= remoteGanttRows) return false;
    try{
      const projected = createRemoteSaveSnapshot(candidate);
      const packed = {
        label:"pre-remote-hydration",
        createdAt:new Date().toISOString(),
        metrics:localMetrics,
        remoteMetrics:{ ...remoteMetrics },
        state:projected
      };
      const written = safeSetLocalItem(V67_RECOVERY_SNAPSHOT_KEY, JSON.stringify(packed));
      if(!written.ok) throw written.error;
      rememberLastGoodSnapshot(projected, "pre-remote-hydration");
      storageStatus.preRemoteHydrationSnapshot = true;
      return true;
    }catch(error){
      recordWarning(`Lokale herstelkopie vóór D1-hydratatie kon niet worden opgeslagen: ${error?.message || error}`);
      return false;
    }
  };

  // V77 compatibility notes for V63 static preflight:
  // V63 legacy compatibility: normal hydration now uses structural validation;
  // the rich-business-data gate is reserved only for explicit corrupt-chunk recovery.
      ...
      try{
        ...
        if(remote?.exists && remote.state && typeof remote.state === "object"){
          ...
          if(stateHasBusinessData(incoming)){
            ...
            storageStatus.label = `Cloudflare D1 - gedeelde planning (${remoteSafetySnapshot.projectCount} projecten)`;
            storageStatus.lastError = null;
            storageStatus.unsynced = false;
    ...
    initPromise = (async() => {
      ...
      try{
        ...
        if(remote?.exists && remote.state && typeof remote.state === "object"){
          ...
          const remoteStateValid = stateIsValidRemoteDocument(incoming);
          if(remoteStateValid){
            preserveLocalCandidateBeforeRemoteHydration(localCandidate, remoteSafetySnapshot);
            state = incoming;
            state.meta = state.meta || {};
            ...
            storageStatus.label = remoteSafetySnapshot.projectCount > 0
              ? `Cloudflare D1 - gedeelde planning (${remoteSafetySnapshot.projectCount} projecten)`
              : "Cloudflare D1 - nieuwe gedeelde planning";
            storageStatus.lastError = null;
            storageStatus.unsynced = false;
            ...
            storageStatus.label = "D1-state ongeldig - lokale herstelmodus";
            storageStatus.unsynced = stateHasBusinessData(localCandidate);
            storageStatus.lastError = "D1-state is structureel ongeldig; lokale data is niet automatisch naar D1 geschreven.";
            recordWarning(storageStatus.lastError);
            state = localCandidate;
          }
        }else{
          // A reachable D1 without an active state is a valid new shared planning.
          // Never upload browser cache implicitly; preserve it as a recovery copy.
          const emptyRemoteState = normalizeState(defaultState());
          const emptyMetrics = stateMetrics(emptyRemoteState);
          preserveLocalCandidateBeforeRemoteHydration(localCandidate, { ...emptyMetrics, version:remoteVersion, bytes:0 });
          state = emptyRemoteState;
          remoteSafetySnapshot = { ...emptyMetrics, bytes:0, version:remoteVersion, loadedAt:new Date().toISOString() };
          storageStatus.mode = "api";
          storageStatus.stateSource = "remote-d1";
          storageStatus.label = "Cloudflare D1 - nieuwe gedeelde planning";
          storageStatus.unsynced = false;
          storageStatus.lastError = null;
          storageStatus.lastSuccessfulD1LoadAt = new Date().toISOString();
          scheduleBootSnapshotPersistence(state, "remote-d1-empty-load");
          markBootPhase("remote-state-ready");
        }
      }catch(error){
  ...
  window.CWS = {
    ...
    syncRemoteVersion: (version) => {
      const next = Number(version || 0);
      if(Number.isInteger(next) && next >= remoteVersion){
        remoteVersion = next;
        storageStatus.remoteVersion = next;
      }
      return remoteVersion;
    },
    persistNow: (reason="manual-persist") => flushRemoteSaveQueue(reason),
    normalizeSafeLogo,
    safeLogoDataUrl,
    retryRemoteSave,
    loadServerVersion,
    ...
    hasPermission,
    setUserRole,
    getCompanyLogo: () => safeLogoDataUrl(state.company?.logo),
