(function(){
  ...
  function printHtml(title, body, options = {}){
    ...
    const rawLogo = String(options.logo || "");
    const logo = (() => {
      const api = cws();
      const safeData = api?.safeLogoDataUrl?.({ dataUrl:rawLogo, name:"print-logo" }) || "";
      if (safeData) return safeData;
      try {
        const parsed = new URL(rawLogo, location.href);
        return parsed.origin === location.origin && !/^javascript:/i.test(parsed.protocol) ? parsed.href : "";
      } catch (_) { return ""; }
    })();
