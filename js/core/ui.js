...
const UI = (() => {
  ...
  const companyPrintInfo = () => {
    ...
    const logo = root.getCompanyLogo?.() || "";
    return { companyName, logo };
  };

  const printA3 = (title, subtitle, html, options = {}) => {
    const { companyName, logo } = companyPrintInfo();
    const logoHtml = logo ? `<img class="print-logo" src="${escapeHtml(logo)}" alt="Bedrijfslogo">` : `<div class="print-logo-placeholder">${escapeHtml(companyName).slice(0,2).toUpperCase()}</div>`;
  ...
  return { toast, statusBadge, statusToColor, weekKey, downloadText, downloadCSV, neutralizeSpreadsheetFormula, printA3, openAppsMenuSafe, openModal, openColumnWizard, openTableManager };
})();
