...
check("V59 Gantt plant uitsluitend op werkbare dagen", read("js/core/store.js").includes("normalizeGanttModelSchedules(draft, deepClone(model))") && read("js/core/store.js").includes("v59-working-days-only") && read("layers/laag4_gantt.html").includes("function workSegments") && (read("layers/laag4_gantt.html").includes("bar-segment") || read("layers/laag4_gantt.html").includes("bar-workday-shell")) && read("layers/laag4_gantt.html").includes("Balk bijgewerkt op werkbare dagen") && fs.readFileSync("package.json", "utf8").includes('"preflight:v59"'));
if(process.exitCode) process.exit(process.exitCode);
...
check("V63 D1 recovery hydrateert remote state en UI-only routing schrijft niet naar D1", (read("functions/api/health.js").includes("internal-test-v63") || read("functions/api/health.js").includes("internal-test-v64")) && read("js/core/store.js").includes("V63: recovery hydration is authoritative") && read("js/core/store.js").includes("UI-only route/tab updates must never trigger a remote D1 PUT") && read("js/core/store.js").includes("empty D1 response must not be repaired by auto-uploading") && fs.readFileSync("package.json", "utf8").includes('"preflight:v63"'));
if(process.exitCode) process.exit(process.exitCode);
check("V64 Gantt werkdagsegmenten gebruiken één dragbare shell per taak", read("functions/api/health.js").includes("internal-test-v64") && read("layers/laag4_gantt.html").includes("bar-workday-shell") && read("layers/laag4_gantt.html").includes("bar-work-segment") && read("layers/laag4_gantt.html").includes("data-workday-shell=\"1\"") && fs.readFileSync("package.json", "utf8").includes('"preflight:v64"'));
