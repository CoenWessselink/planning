...
const checks = [
  ...
  [fs.readFileSync("css/ui/01-reset.css", "utf8").includes("#cwsV37MobileActionDock") && !fs.readFileSync("js/core/responsive.js", "utf8").includes("cwsV37MobileActionDock"), "V37 legacy mobiele action dock onderdrukt"],
  ...
  [gantt.includes('id="mobileAddTask"') && gantt.includes('id="mobileProjectSel"') && capacity.includes('data-testid="mobile-capacity-workbar"') && settings.includes("quickLogo"), "V37/V100 mobiele snelacties per module aanwezig"],
...
check("V57 D1 Worker resource hardening aanwezig", read("functions/api/health.js").includes("v57-lightweight-no-state-load") && !read("functions/api/health.js").includes("ensureSchema") && read("functions/api/state.js").includes("rawStateResponse") && read("functions/api/state.js").includes("X-CWS-State-Payload") && read("js/core/store.js").includes("payload=raw-state") && read("js/core/store.js").includes("data.state = JSON.parse(data.stateJson)") && fs.readFileSync("package.json", "utf8").includes('"preflight:v57"'));
if(process.exitCode) process.exit(process.exitCode);
...
check("V86 Capaciteit range en recalc-contract", read("layers/laag5_capaciteit.html").includes("applyDefaultCapacityRange") && read("layers/laag5_capaciteit.html").includes("const start=current") && read("layers/laag5_capaciteit.html").includes("const end=addWeeks(current.year,current.week,25)") && read("layers/laag5_capaciteit.html").includes("function capacityPrintWeeks") && read("layers/laag5_capaciteit.html").includes("addWeeks(tw.year,tw.week,-3)") && read("layers/laag5_capaciteit.html").includes("addWeeks(tw.year,tw.week,26)") && read("js/core/store.js").includes("recalculateCapacityFromGantt"));
