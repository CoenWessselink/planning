...
check(
  'V59 saveProjectGantt corrigeert voor opslaan/capaciteit',
  store.includes('normalizeGanttModelSchedules(') &&
  store.includes('deepClone(model)') &&
  store.includes('{ preserveExplicitRange }') &&
  store.includes('v59-working-days-only')
);
...
check(
  'V59/V65 Gantt corrigeert/deriveert bestaande planning veilig',
  gantt.includes('normalizeModelSchedulesForWorkdays') &&
  gantt.includes('effectiveScheduleMap') &&
  gantt.includes('effectiveTaskSchedule') &&
  gantt.includes('derived:true') &&
  !/function\s+render\s*\([^)]*\)\s*\{[\s\S]{0,8000}saveModel\s*\(/.test(gantt)
);
...
check('V59/V65 Gantt balkweergave behoudt werkdaglogica zonder smalle markers', gantt.includes('function workSegments') && ((gantt.includes('bar-segment') || gantt.includes('bar-workday-shell')) && gantt.includes('segment-first') && gantt.includes('segment-last') || gantt.includes('bar-continuous-task') && gantt.includes('data-v65-continuous-task')));
