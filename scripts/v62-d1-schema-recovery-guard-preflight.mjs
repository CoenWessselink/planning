...
const checks = [
  ...
  ['store catastrophic overwrite guard', /protectAgainstCatastrophicOverwrite/.test(files.store) && /v62-catastrophic-overwrite/.test(files.store)],
  ['server D1 save guard', /assertNoCatastrophicOverwrite/.test(files.state) && /V62 D1 save guard/.test(files.state)],
  ...
  ['preflight v62 metrics', /V62 state metrics/.test(files.preflight) && /V62 D1 overwrite guard actief/.test(files.preflight)],
