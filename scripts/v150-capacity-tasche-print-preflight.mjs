...
add("Index laadt nieuwe printmodule met cachebust", index.includes("js/core/capacity_print_tasche_a3.js?v=161"));
add("Capaciteit-laag laadt printmodule direct in eigen document", cap.includes('../js/core/capacity_print_tasche_a3.js?v=161'));
...
add("Module heeft eigen capaciteit-printroot en marker", mod.includes("CWS_CAPACITY_TASCHE_A3_PRINT_V161") && mod.includes("cwsCapacityPrintRoot") && mod.includes('data-cws-print-kind="capacity-overview"'));
...
add("Capaciteit print via dedicated printpagina met postMessage", mod.includes("/layers/capacity_print_view.html") && mod.includes("transport=message") && mod.includes(".open(printUrl, PRINT_WINDOW_NAME)") && mod.includes("postMessage(payload, targetOrigin)") && mod.includes("cws_capacity_overview_print"));
...
add("Capaciteit print-HTML gaat niet meer via localStorage quota", !mod.includes("PRINT_STORAGE_PREFIX") && !mod.includes("localStorage.setItem(key, printHtml)") && printView.includes("CWS_CAPACITY_PRINT_HTML") && printView.includes("CWS_CAPACITY_PRINT_VIEW_READY"));
add("Dedicated printpagina bevat alleen capaciteitsoverzicht", printView.includes("CWS_CAPACITY_TASCHE_A3_PRINT_V161") && printView.includes("CAPACITEITSOVERZICHT") && printView.includes("renderPrintHtml") && printView.includes("window.print()") && !/CWS_BWS_A3_PRINT_FINAL|data-bws-marker|Bouwplanning/.test(printView));
...
add("Overzicht per afdeling toont benodigd/beschikbaar/restant per week", mod.includes("availableWeeks") && mod.includes("remainingWeeks") && mod.includes("BENODIGD PER WEEK") && mod.includes("BESCHIKBAAR PER WEEK") && mod.includes("RESTANT PER WEEK"));
add("Plus/min totalen krijgen duidelijke kleurvlakken", mod.includes(".metric.pos,.week-cell.pos{color:#047857") && mod.includes("background:#ecfdf5") && mod.includes(".metric.neg,.week-cell.neg{color:#b91c1c") && mod.includes("background:#fef2f2"));
