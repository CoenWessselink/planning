...
const V101_MARKER = "v101-d1-state-save-revision-recovery";
...
function corsResponse(status = 204) {
  return new Response(null, {
    ...
    headers: {
      ...
      "Access-Control-Expose-Headers": "X-CWS-OK,X-CWS-State-Exists,X-CWS-Version,X-CWS-Updated-At,X-CWS-Updated-By,X-CWS-User-Email,X-CWS-User-Role,X-CWS-User-Display-Name,X-CWS-Bytes,X-CWS-Chunked,X-CWS-Chunked-Manifest,X-CWS-Chunk-Index,X-CWS-Chunk-Count,X-CWS-V85,X-CWS-V101"
    }
  });
}
...
function buildChunkManifest(version, bytes, chunkCount, updatedBy) {
  return JSON.stringify({
    ...
    marker: V82_MARKER,
    saveRecoveryMarker: V101_MARKER,
    schemaVersion: 12,
    tenantId: TENANT_ID,
    stateKey: STATE_KEY,
    version,
...
function parseChunkManifest(raw) {
  ...
  try {
    ...
    if (parsed?.__cwsChunkedState && (parsed?.marker === V82_MARKER || parsed?.saveRecoveryMarker === V101_MARKER)) return parsed;
  } catch (_) {}
  return null;
}
...
function splitStateIntoChunks(stateJson) {
  const chunks = [];
  for (let i = 0; i < stateJson.length; i += CHUNK_CHAR_SIZE) {
    chunks.push(stateJson.slice(i, i + CHUNK_CHAR_SIZE));
  }
  return chunks;
}
...
async function readStateChunkResponse(db, manifest, row, url) {
  ...
  if (index < 0 || index >= Number(manifest?.chunkCount || 0)) {
    return json({ ok: false, error: "Ongeldige chunk-index.", v101: V101_MARKER }, 400);
  }
  ...
  if (requestedVersion !== manifestVersion) {
    return json({ ok: false, error: `Chunkversie komt niet overeen (${requestedVersion} != ${manifestVersion}).`, v101: V101_MARKER }, 409);
  }
  ...
  if (!chunk?.chunk_text) return json({ ok: false, error: `State chunk ${index} ontbreekt.`, v101: V101_MARKER }, 500);
...
function assertIncomingIsSafe(incomingRaw) {
  ...
  if (looksLikeDemoOrEmpty) {
    const error = new Error(`Opslaan geblokkeerd: inkomende state lijkt leeg/demo (${incomingProjects} projecten/${incomingRows} Gantt-rijen).`);
    error.status = 409;
    error.guard = "v101-empty-state-save-guard";
    error.incomingMetrics = incoming;
    throw error;
  }
  return incoming;
...
async function readIncomingState(context) {
  ...
  const bytes = textByteLength(raw);
  if (bytes > MAX_STATE_BYTES) {
    const error = new Error("State payload is te groot.");
    error.status = 413;
    throw error;
  }
  const rawMode = context.request.headers.get("X-CWS-State-Payload") === "raw-state" ||
    url.searchParams.get("payload") === "raw-state";
  if (rawMode) {
    ...
    return { stateJson: raw, baseVersion: parseBaseVersion(context, url), bytes, rawMode: true };
  }
  let body;
  try {
    body = JSON.parse(raw);
...
async function cleanupOldChunksBestEffort(db, nextVersion) {
  try {
    await db.prepare(
      `DELETE FROM app_state_chunks WHERE tenant_id = ? AND state_key = ? AND version <> ?`
    ).bind(TENANT_ID, STATE_KEY, nextVersion).run();
  } catch (_) {}
}

async function writeFullStateJsonSequential(db, stateJson, nextVersion, email) {
  await ensureChunkSchema(db);
  ...
  if (bytes <= CHUNK_THRESHOLD_BYTES) {
    ...
    await cleanupOldChunksBestEffort(db, nextVersion);
    return { chunked: false, chunkCount: 0, bytes, marker: V101_MARKER };
  }
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    await db.prepare(
      `INSERT OR REPLACE INTO app_state_chunks (tenant_id, state_key, version, chunk_index, chunk_text)
       VALUES (?, ?, ?, ?, ?)`
    ).bind(TENANT_ID, STATE_KEY, nextVersion, index, chunks[index]).run();
  }
  ...
  await db.prepare(
    `INSERT INTO app_state
     ...
     ON CONFLICT(tenant_id, state_key) DO UPDATE SET
       ...
       updated_at = CURRENT_TIMESTAMP,
       updated_by = excluded.updated_by`
  ).bind(TENANT_ID, STATE_KEY, manifest, nextVersion, email).run();

  await cleanupOldChunksBestEffort(db, nextVersion);
  return { chunked: true, chunkCount: chunks.length, bytes, marker: V101_MARKER };
}

async function auditBestEffort(db, email, action, metadata, entityType, entityId) {
  try {
    await writeAudit(db, email, action, metadata, entityType, entityId);
  } catch (_) {}
}

async function handleGet(context) {
  const db = context.env?.DB;
  if (!db) return json({ ok: false, error: "D1-binding DB ontbreekt.", v101: V101_MARKER }, 500);
  const email = requireActorEmail(context.request);
  if (!email) return json({ ok: false, error: "Cloudflare Access-identiteit ontbreekt.", v101: V101_MARKER }, 401);
  ...
  try {
    ...
    if (!schema.ok) return json({ ok: false, error: "D1-schema kon niet automatisch worden hersteld.", schemaErrors: schema.errors, v101: V101_MARKER }, 500);

    const user = await getOrCreateUser(db, email);
    if (!user.active) return json({ ok: false, error: "Gebruiker is inactief.", v101: V101_MARKER }, 403);
    ...
    if (exists && manifest && (rawResponse || wantsChunkManifestResponse(context, url))) {
      return chunkManifestResponse(manifest, row, user, email);
    }
    ...
    return json({
      ...
      v101: { marker: V101_MARKER, middleware: true, rawStateResponse: false }
    });
  } catch (error) {
    return json({ ok: false, error: error.message, v101: V101_MARKER }, error.status || 500);
  }
}
...
async function handlePut(context) {
  ...
  try {
    ...
    const user = await getOrCreateUser(db, email);
    if (!user.active) return json({ ok: false, error: "Gebruiker is inactief.", v101: V101_MARKER }, 403);
    if (!canWriteState(user)) return json({ ok: false, error: "Viewer heeft alleen leesrechten.", v101: V101_MARKER }, 403);

    const url = new URL(context.request.url);
    const incoming = await readIncomingState(context);
    const incomingMetrics = assertIncomingIsSafe(incoming.stateJson);

    const current = await db.prepare(
      `SELECT version, state_json, updated_by FROM app_state WHERE tenant_id = ? AND state_key = ?`
    ).bind(TENANT_ID, STATE_KEY).first();
    const currentVersion = Number(current?.version || 0);
    const baseVersion = Number(incoming.baseVersion ?? 0);
    const sameActor = String(current?.updated_by || "").toLowerCase() === String(email || "").toLowerCase();
    const softConflictRecovered = currentVersion > 0 && baseVersion !== currentVersion && sameActor && baseVersion > 0 && (currentVersion - baseVersion) <= 10;

    if (currentVersion > 0 && baseVersion !== currentVersion && !softConflictRecovered) {
      return json({
        ok: false,
        error: "State is gewijzigd door een andere gebruiker.",
        currentVersion,
        baseVersion,
        v101: { marker: V101_MARKER, softConflictRecovered: false }
      }, 409);
    }

    const nextVersion = currentVersion + 1;
    const writeResult = await writeFullStateJsonSequential(db, incoming.stateJson, nextVersion, email);

    await auditBestEffort(db, email, "state_saved", {
      version: nextVersion,
      baseVersion,
      bytes: incoming.bytes,
      rawMode: incoming.rawMode,
      incomingMetrics,
      softConflictRecovered,
      v101: { marker: V101_MARKER, sequentialChunkSave: true, auditBestEffort: true }
    }, "app_state", STATE_KEY);
    ...
    return json({
      ...
      updatedBy: email,
      bytes: incoming.bytes,
      v60: { rawStateSave: incoming.rawMode },
      v82: { chunkedStateSave: true, chunked: writeResult.chunked, chunkCount: writeResult.chunkCount, marker: V82_MARKER },
      v101: { marker: V101_MARKER, middleware: true, sequentialChunkSave: true, softConflictRecovered }
    });
  } catch (error) {
    return json({
      ok: false,
      ...
      v101: { marker: V101_MARKER, middleware: true, recovered: false }
    }, error.status || 500);
  }
}
...
export async function onRequest(context) {
  ...
  if (context.request.method === "OPTIONS") return corsResponse(204);
  if (context.request.method === "GET") return handleGet(context);
  if (context.request.method === "PUT") return handlePut(context);
  return json({ ok: false, error: "Method not allowed.", v101: V101_MARKER }, 405);
}
