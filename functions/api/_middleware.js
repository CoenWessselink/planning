...
function chunkManifestWanted(c,u) { return u.searchParams.get("chunks")==="auto" || u.searchParams.get("chunks")==="manifest" || c.request.headers.get("X-CWS-State-Response")==="chunk-manifest"; }
...
function rawHeaders({exists,version,at,by,user,email,bytes,journal=false,mutationId="",chunked=false,chunkCount=0,manifest=false}) { return {"X-CWS-OK":"true","X-CWS-State-Exists":exists?"1":"0","X-CWS-Version":String(Number(version||0)),"X-CWS-Updated-At":safe(at||""),"X-CWS-Updated-By":safe(by||""),"X-CWS-User-Email":safe(user?.email||email),"X-CWS-User-Role":safe(user?.role||"viewer"),"X-CWS-User-Display-Name":safe(user?.display_name||user?.email||email),"X-CWS-Bytes":String(Number(bytes||0)),"X-CWS-Chunked":chunked?"1":"0","X-CWS-Chunked-Manifest":manifest?"1":"0","X-CWS-Chunk-Count":String(Number(chunkCount||0)),"X-CWS-Journal-Recovered":journal?"1":"0","X-CWS-Mutation-Id":safe(mutationId),"X-CWS-V102":MARKER}; }
